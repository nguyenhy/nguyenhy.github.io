...
meta:
  ...
  url: https://nguyenhy.github.io/blogs/vod-simulate-live/1738907578230/
  keywords:
    - blog
    - me
    - hyn
  article:
    published_time: 1738907578230
---
...
HLS (HTTP Live Streaming) operates using two main types of M3U8 playlists
#### Master playlist
A Master Playlist is an M3U8 file that tells the HLS player which video quality options are available. 
...
Example of master playlist contains:
...
<img loading="lazy" src="/assets/blogs/vod-simulate-live/1738907578230/debug-load-playlist.png" alt="debug load playlist" />
...
function process(playlist) {
  if (playlist.includes('#EXT-X-STREAM-INF')) {
    // we onyl process media playlist
    return playlist 
  }

  const output = playlist

  // assume current live-stream should playing around 15.7s
  const POSITION = 15.7

  // 1. parse playlist (string) to array of lines
  // use https://www.npmjs.com/package/m3u8-parser
  const parsed = new m3u8Parser.Parser()
  parsed.push(playlist)

  // 2. find the segment at specific time (SegmentX)
  ...
  // 3. remove all semgent after `SegmentX`
  if (stopIndex !== null) {
    if (segments[stopIndex + 1]) {
      /**
       * playlist:
       * ```
       * #EXTINF:10,
       * segment0.ts
       * #EXTINF:10,  
       * segment1.ts  <----stopIndex
       * #EXTINF:10,  
       * segment2.ts  <----stopIndex + 1
       * #EXTINF:10,
       * segment3.ts
       * ```
       * 
       ...
       * ```
       */
      const splitted = playlist.split(segments[stopIndex + 1].uri) 

      output = splitted[0];
    }
  }

  // 4. Remove VOD tag indicator
    output = output
...
## Demo
<iframe src="/assets/blogs/vod-simulate-live/1738907578230/demo.html" style="width: 100%; height: 350px"></iframe>
Available demo at [here](https://output.jsbin.com/mavesos)
